...
function disableTaskDetailsCheck() {
	...
	if (contextType == "wontdo") {
		const taskCheckboxes = document.querySelectorAll(
			".task-date-status>.task-checkbox"
		);
		taskCheckboxes.forEach((box) => {
			box.disabled = true;
		});
...
function disableTaskDetailsEdit() {
	...
	if (contextType == "deleted") {
		const unselectableElements = document.querySelectorAll(
			".task-date-status, .task-header, .task-description, .task-checklist-container, #list-selection"
		);
...
function enableAddBar() {
	...
	const schedule_lists_FilterBtns = document.querySelectorAll(
		".schedule-filter>button , .lists-filter>button"
	);
...
function updateChecklist() {
	...
	checklistItems.forEach((item) => {
		const selectedTask = document.querySelector(".task.selected");
		const taskSrc = logic.getTaskById(parseInt(selectedTask.dataset.id));
		const itemIndex = taskSrc.getChecklistItemIndexById(
...
function updateTaskDetails() {
	...
	tasks.forEach((task) => {
		task.addEventListener("click", () => {
			...
			taskChecklist.addEventListener("click", () => {
				if (taskSrc.checklist.length == 0) {
					...
					taskSrc.checklist.forEach((item) => {
						if (item.name?.trim()) {
							if (
								taskSrc.checklist.indexOf(item) != 0 ||
			...
			} else if (contextType != "deleted" && contextValue != "Completed") {
